...
<section class="content">
  ...
  <!-- /.box-body -->
        ...
        <div class="box">
  <div class="box-header">
    <h3 class="box-title"><?= __('Related Sub Categories') ?></h3>
  </div>
  <!-- /.box-header -->
  <div class="box-body no-padding">
    ...
    <table class="table table-condensed">
      ...
      <tbody>
             <?php foreach ($category->subcategories as $childCategories): ?>
            <tr>
                ...
                <td><?= h($childCategories->created) ?></td> 
                  <td class="actions">
                   <?= $this->Html->link(
                        ...
                        ["controller"=>'subcategories','action' => 'view', $childCategories->id],
                        ['escape' => false, 'title' => __('View'), 'class' => 'btn btn-info btn-xs']
                    ) ?>  
                    <?= $this->Html->link(
                        '<span class="fa fa-pencil"></span><span class="sr-only">' . __('Edit') . '</span>',
                        ["controller"=>'subcategories','action' => 'edit', $childCategories->id],
                        ['escape' => false, 'title' => __('Edit'), 'class' => 'btn btn-success btn-xs']
                    ) ?>
                  
                     
                    <?= $this->Form->postLink(__('Delete'), ["controller"=>'subcategories','action' => 'delete', $childCategories->id], ['confirm' => __('Are you sure you want to delete # {0}?', $childCategories->id),'class' => 'btn btn-danger btn-xs']) ?>
                </td>
            </tr>
            <?php endforeach; ?>  
      </tbody>
